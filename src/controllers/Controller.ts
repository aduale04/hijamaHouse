...
class Controller {
  ...
  private initRoutes() {
    ...
    this.router.get('/cancel-booking/:id/:dateBooking/:email/:phoneNumber', this.cancelBooking);
  }

  private home = (req: Request, res: Response, _next: NextFunction) => {
    res.render('index', {
      title: 'Hijamahouse - Home',
      description: "",
  ...
  book = (req: Request, res: Response, _next: NextFunction) => {
    ...
    books.findAvailableTimeByDate(date.toISOString().substr(0, 10), ({success, message, data}: any) => {
      ...
      if (data) {
        ...
        res.render('booking', {
          title: 'Hijamahouse - Booking form',
          description: "",
          image: 'img/logo/logo.png',
          book: 'book',
  ...
  cancelBooking = (req: Request, res: Response, _next: NextFunction) => {
    const {phoneNumber, id, email, dateBooking} = req.params
    req.body.id = parseInt(id);
    req.body.phoneNumber = phoneNumber;
    req.body.email = email;
    req.body.dateBooking = dateBooking;
    const booking = new Booking(req.body);

    booking.cancelBooking((result: any) => {
      res.redirect(`${process.env.WEB_URL}/book?cancelMessage=${result.message}`);
    })
